...
- **Three third-party models read repository source, and a run costs real money.** → No egress path, no credential access, allowlisted environment, no session persistence. Cost is bounded per reviewer by a wall-clock timeout, with an optional output-token ceiling when one is configured, computed from catalog rates, and reported per reviewer and in total.
